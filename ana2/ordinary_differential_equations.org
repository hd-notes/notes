* Gewöhnliche Differentialgleichungen
  ...
  *Methode der Variation der Konstanten* \\
  ...
  (Separation der Variablen). Sei $y(t)$ eine Lösung mit $c = 1$.
  ...
  *Existenzsatz von Peano* \\
  ...
  #+end_thm
  #+begin_proof latex
  Mit Hilfe der Differenzenmethode konstruieren wir eine Folge von stückweise linearen Funktionen, welche eine Teilfolge besitzt, die (gleichmäßig) gegen eine Lösung des Anfangswertproblems konvergiert.
  ...
  *Schritt 3* Es bleibt zu zeigen, dass die Grenzfunktion $x$ der Integralgleichung genügt.
  ...
  #+begin_thm latex
  Sei $x$ eine Lösung der Anfangswertaufgabe auf dem Intervall $I$. Falls $f ∈ C^m(D)$ für ein $m \geq 1$ ist, dann $x ∈ C^{m + 1}(I)$
  #+end_thm
  #+begin_proof latex
  ...
  Durch wiederholte Anwendung dieses Argument folgt die Behauptung.
  #+end_proof
  *Eindeutigkeit?*
  #+begin_ex latex
  ...
  #+end_proof
  #+begin_remark latex
  Die Lösung $x^{\ast}$ erhält man durch im Banachraum $V = C(I_ε)$ konvergente Fixpunktiteration (sogenannte "sukzessive Approximation")
  ...
  $x' = ax$ $⇒$ keine gleichmäßige Beschränktheit.
  \[x' = \frac{a x}{t + x} x \leq a x\]
  $⇒ x(t) \leq x(t) e^{at}$ $⇒$ globale Lösungen existieren.
  #+end_ex
  ...
  $⇒ x(t)$ bleibt beschränkt für $t ∈ (0, T_+(x_0))$ und kann daher	fortgesetzt werden. Damit folgt $T_+(x_0) = T_2$. Analog erhält man $T_{-}(x) = T_1$
  #+end_proof
  ...
  #+end_proof
  #+begin_remark latex
  Aus der Bedingung folgt, dass die durch den Existenzsatz von Peano und den Fortsetzungssatz gelieferte lokale Lösung $x$ eindeutig bestimmt ist.
  #+end_remark
  #+begin_proof latex
...
** Lineare Systeme
   ...
   #+end_thm
   #+begin_proof latex
   1. Übung: Die Addition ist komponentenweise definiert, zum Beispiel für $α, β ∈ ℝ, u, v ∈ H$
	  \[⇒ (α u + β v)'(t) = α u'(t) + β v'(t) = A(t)(α u + β v)(t)\]
   2. Seien $\{u_0^1, \dots, u_0^n\}$ eine Basis von $ℝ^n$, $\{u^1, \dots, u^n\}$ zugehörige Lösungen mit $u'(t_0) = u_0^i$. Lineare Unabhängigkeit:
	  Seien $α_i ∈ ℝ$ mit
   ...
   $Φ = (φ^1, \dots, φ^n)$ in den Spaltenvektoren $φ^i$ schreiben. Nach Satz 3.15 ist $Φ(t)$ für jedes $t \geq t_0$ invertierbar und es gilt
   \[Φ'(t) = A(t) Φ(t)\]
   ...
   #+end_thm
   #+begin_proof latex
   Differentiation liefert mit Produktregel
   \begin{align*}
   ...
   $c_1, c_2$ kann man aus den Anfangsdaten finden. Das qualitative Verhalten der Lösung ist von Vorzeichen $λ_1, λ_2$ abhängig.
   ...
   \end{align*}
   Die gut erkennbare Struktur der einzelnen Komponenten (als Produkt aus Polynomen und Exponentialfunktion) lässt sich durch vollständige Induktion für Systeme mit beliebig vielen linearen Gleichungen nachweisen.
   #+end_ex
   #+begin_lemma latex
   ...
   2. Seien $v_i ∈ ℝ^n \setminus \{0\}$, $n$ linear unabhängige Eigenvektoren mit Eigenwerten $λ_i ∈ ℝ$. Dann bilden die Funktionen $v_i e^{λ_i t}$ eine Basis von $\mathcal{L}$
   ...
   Die Blöcke $J_k$	haben für ein $λ_i ∈ ℂ$ die Form
   ...
   #+end_proof
   Mit Hilfe der Jordanschennormalform lässt sich die Exponentialfunktion von Matrizen ausrechnen. Die Anwendung der Exponentialfunktion auf
...
** Asymptotisches Lösungsverhalten bei Differentialgleichungen
   ...
   der Differentialgleichung $x'(t) = f(x(t))$ falls es eine offene Umgebung $U$ von $x_0$ gibt, dass
   für jedes $x_0 ∈ U$ die Lösung der Gleichung gegen $\bar x$ konvergiert, das heißt
   ...
   \end{align*}
   $⇒$ Bifurkation Diagramm (Verzweigung).
   #+end_ex
   #+begin_defn latex
   ...
   #+end_remark
   #+begin_defn latex
   $\bar x$ sei ein stationärer Punkt einer autonomen Differentialgleichung $x' = f(x)$ mit $f: ℝ^n \to ℝ^n$. $\bar x$ heißt stabil (im Sinne von Lyapunov (Ljaupnow))
   ...
   \[\abs{x(t) - \bar x} < ε ∀ t > 0\]
   $\bar x$ heißt asymptotische stabil, wenn $\bar x$ stabil ist und zusätzlich
   ...
   #+end_proof
   Aus der Theorie der linearen Differentialgleichungen folgt:
   #+begin_lemma latex
   ...
   *Zusammenfassung für lineare Systeme* \\
   ...
   4. Falls vielfache $λ_i ⇒$ Jordan Blöcke $⇒$ polynomielle Komponenten in der Lösung
   ...
   \end{align*}
   Aus Voraussetzung $\lim_{x \to 0} \abs{g(x)} / \abs{x} = 0$ gibt es einen Radius $ρ > 0$ mit $\abs{g(z)} \leq α / (2c) \abs{z} ∀ z ∈ \bar K_ρ(0)$. Wir betrachten $x' = Ax + g(x)$ mit $\abs{x(0)} \leq ρ / (2(1 + c))$.
   ...
   wenn die rechte Seite differenzierbar ist. Denn nach Definition von Totaler Ableitung erfüllt die Restfunktion $φ_{\bar x}(·)$ in
   ...
   - Falls es eine Lyapunov-Funktion auf einer offenen Umgebung $U$ von $\bar x$ gibt mit $L(\bar x) = 0, L(x) > 0$ für $x \neq \bar x$, dann ist
     $\bar x$ stabil
   - Falls es eine strikte Lyapunov-Funktion auf einer Umgebung $U$ von $\bar x$ mit $L(\bar x) = 0, L(x) > 0$ für $x \neq \bar x$ und $L'(x) < 0$, dann
