...
** Lineare Systeme
   ...
   #+end_proof
   #+begin_thm latex
   Zu einer beliebigen Matrix $A ∈ ℝ^{n × n}$ existiert invertierbare Matrix $S$ sodass die Matrix $S^{-1} A S$ die *Jordannormalform* hat, das heißt
...
** Asymptotisches Lösungsverhalten bei Differentialgleichungen
   ...
   Das zugehörigen Anfangswertproblem mit $x(0) = 0$ lässt sich lösen durch Seperation der Variablen. Für $λ < 0$ $x(t) = - \sqrt{\abs{λ}} \tanh(t \sqrt{\abs{λ}})$.
   Für $λ = 0 ⇒ x(t) = 0$.
   \begin{align*}
   λ > 0 &⇒ f > 0 \\
   λ < 0 &  \\
   ...
   \end{align*}
   $⇒$ Bifukation Diagram (Verzweigung).
   #+end_ex
   #+begin_defn latex
   ...
   $(\bar x, λ_0)$ eine *Verzweigung* (Bifukation) wenn	gilt:
   Die Anzahl von stationären Punkten von $x' = f(x, μ_k) ∈ K_r(\bar x)$ ist ungleich der Anzahl stationärer Funkte von
   $x' = f(x, ν_k) ∈ K_r(\bar x)$ für zwei Folgen $(μ_n)_{n ∈ ℕ}, (ν_n)_{n ∈ ℕ}$ in $Λ$ die gegen
   $λ_n$ konvergieren, für jede Kugel $K_r(\bar x) ⊂ ℝ^n$ und hinreichend grohen $n ∈ ℕ$.
   In unserem Beispiel
   \begin{align*}
   ...
   #+end_defn
   #+begin_remark latex
   Auf der Suche nach Bifukationen gehst es also um die Lösung von $f(x, λ) = 0$ mit einem Parameter $λ$.
   ...
   Notwendige Bedingung für Bifukation: $x' = f(x, λ)$ in $(\bar x, λ_0)$ eine Bifukation besitzt dann kann die partielle Ableitung $δ_1 f(\bar x, λ_0): ℝ^n \to ℝ$ nicht
   invertierbar sein.
   #+end_remark
   #+begin_defn latex
   ...
   die Forderung
   \[\lim_{t \to ∞} x(t) = \bar x\]
   erfüllen. $\bar x$ heitß instabil wenn $\bar x$ nicht stabil ist.
   #+end_defn
   #+begin_lemma latex
   ...
   Differentialgleichung $y' = A y + b$ ebefalls stabil.
   #+end_lemma
   #+begin_proof latex
   ...
   #+end_thm
   #+begin_proof latex
   $v_0$ sei Eigenvektor zu $λ$ (mit $\Re λ > 0$) und $\abs{v_0} \leq r$. $t \to e^{λ t} v_0$ ist eine Lösung (komplux) von $x' = A x$. Dabei strebt
   ...
   *Zusammenfassung für lineare Systeme* \\
   ...
   Dann ist der Nullpunkt instabil bezüglich der Differntialgleichung
   \[x' = A x + g(x)\]
   #+end_thm
   (ohne Beweis)
   #+begin_remark latex
   ...
   #+end_remark
   #+begin_korollar latex
   $f: ℝ^n \to ℝ^n$ sei differnzierbar und besitze einen stationären Punkt $\bar x ∈ ℝ^n$. Dann gilt
   ...
   #+end_defn
   #+begin_remark latex
   Das Richtungsfeld gibt den Verlauf der Trajektorien an. Der exakte Verlauf der Frajektorie ist für ein System
