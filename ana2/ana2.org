...
* Gewöhnliche Differentialgleichungen
  ...
  *Bewerkung zur Notation* \\
  ...
  \[\dd{}{t} x(t) = f(t) ∀ t ∈ ℝ\]
  Wir müssen noch die Position zu irgendwelchen Zeitpunkt nennen. Das heißt die Lösung ist nicht eindeutig solag wir keinen Wert $x(t_0) ∈ ℝ$ festlegen.
  Das Problem
  \begin{align*}
  \dd{}{t} x(t) &= f(t) \\
  x(t_0) &= x_0
  ...
  *Ziel*:
  ...
  #+end_ex
  #+begin_defn latex
  Gegeben seien nicht leere Teilmenge $D ⊂ ℝ × ℝ^m$ und eine Funktion $f: D \to ℝ^m$. Dann nennt man
  \[x' = f(·, x)\]
  ...
  *Methode der Trennung der Variablen* \\
  ...
  Mit Hilge die Substitution $z := x(s)$ ergibt sich (mit $\dd{z}{x} = x'(s)$)
  ...
  \end{axis}
  \end{tikzpicture}
  Dies ist keine *globale* ($∀t ∈ ℝ_+$) Lösung, da man $x(t)$ mich nach $t = t^{\ast}$ vorsetzen kann.
  #+end_ex
  *Methode der Variation die Konstanten* \\
  ...
  mit einer freien Konstante $r ∈ ℝ$. Damit wird
  ...
  *Schritt 3* Es bleibt zu zeigen, dass die Grenzfunktion $x$ der Integralgleichung genügt.
  ...
  und weiter für
  \[\abs{t - t'} < δ_ε, \norm{x - x'} < ε' ⇒ \norm{f(t, x) - f(t', x')} < ε\]
  Für hinreichend grohes $i \geq i_ε$ (das heißt hinreichend kleines $h_i$) folgt damit
  \begin{align*}
  ...
  Dann ist die lokale Lösung $x$ nach rechts und nach linkt auf ein maximales Existenzintervall $I_{max} = (t_0 - T_{\ast}, t_0 + T_{\ast})$ (stetig differenzierbar)
  fortsetzbar, solange der $\Graph(x)$ nicht auf dem Rang von $D$ stößt. Dabei kann $\Graph(x) := \{(t, x(t) \mid t ∈ I_{max})\}$ unbeschränkt sein sowohl durch $t \to t_0 + T^{\ast} = ∞$ als auch $\norm{x(t)} \xrightarrow{t \to t_0 + T^{\ast}} 0$
  #+end_thm
  #+begin_proof latex
  Ohne Beschränkung der Allgemeinheit behalten wir nur $[t_0, t_0 + T_{\ast}]$. Peano Satz liefert Existenz einer Lösung $x^0$ auf $[t_0, t_1], t_1 := t_0 + T_0$ mit
  ...
  *Eindeutigkeit?*
  ...
  has heißt $x^{\ast}$ löst die Integralgleichung.
  #+end_proof
  #+begin_remark latex
  ...
  x(0) &= x_0
  \end{align*}
  wir bekommen $n$ Gleichung. Es gilt für $t < ε(x)$:
  \begin{align*}
  ...
  \end{align*}
  Tatsächlich konvergierte die Reiche kann gleidweise nach $t$ differenziert werden, daher stellt sie die
  Lösung da.
  #+end_ex
  #+begin_remark latex
  ...
  $x' = ax$ $⇒$ keine gleichmäßige Beschänktheißt.
...
** Lineare Systeme
   ...
   #+end_proof
   #+begin_remark latex
   Ist $u(t_0)$ nicht vorgeschrieben, ergeben sich Lösungen der inhomogenen Gleichung als Summe homogener Lösungen $u^i ∈ H$ und einer speziellen Lösung der inhomogenen Gleichung als Summe homogener Lösungen
   $u^i ∈ H$ und einer speziellen Lösung der inhomogenen Gleichung. Zum Beispiel:
