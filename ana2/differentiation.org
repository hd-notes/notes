* Stetigkeit und Differenzierbarkeit im $ℝ^n$
  ...
  *Taylor-Entwicklung und Extremwerte in $ℝ^n$*
  ...
  #+begin_defn latex
  Ein n-dimensionaler *Muliindex* ist ein Tupel $α= (α_1, \dots, α_n)$ mit $α_i ∈ ℕ$. Für Multiindizes sind die *Ordnung* $\abs{α}$ und die Fakultät $α!$
  definiert durch
  \begin{align*}
  ...
  #+end_defn
  #+begin_remark latex
  Wegen der Stetigkeit der Ableitung ist dieser Ausdruck unabhänig von der Reihenfolge der partiellen Ableitungen.
  Wir definieren
  \[\sum_{\abs{α} = 0}^{r} a_α := \sum_{k = 0}^{r} \sum_{\substack{α ∈ ℕ^n \\ \abs{α} = k}} a_α\]
  #+end_remark
  #+begin_ex latex
  Für $n = 3$ sind die Multiindizes $α = (α_1, α_2, α_3)$ der Ordnung $\abs{α} = 2$ gegeben durck
...
** Satz über implizite Funktionen und der Umkehrsatz
   ...
   $\pp{F}{y}$ in jedem Punkt $(x, (φ(x))), x ∈ V_1$ invertierbar ist. Wir zeigen de Differenziebarkeit von $φ$ nur in 0.
   ...
   Es muss also gezeigt werden, dass $ω(x, φ(x)) = \mathcal{o}(\norm{(x, φ(x))})$. Zeige dazu, dass es eine Umgebung $V_1 ⊂ V_1$ von $0$ gibt und eine Konstate $K > 0$, sodass
   ...
   Wegen der Stetigkeit von $φ$ gibt es eine Nullumgebung $V_1' ⊂ V_1$, sodass der Graph $φ\mid_{V_1'}$ ganz in $V'$ enthalten ist. Dahit gilt
   ...
   ($\hat y = f(\hat x)$) abgebildet wird. Die Umkehrabbildung ist ebenfalls regulär in $\hat y$. $f^{-1}: U(\hat y) \to V(\hat x)$. Für die Funktionalmatrix und -determinante gilt:
   ...
   $V(\hat x)$ offen. Fermer wird $V(\hat x)$ bijktiv von $f$ abgebildet mit zugehörgen Umkehrabbildung $f^{-1} = φ$. Wegen $J_{f \circ f^{-1}} = J_{\id} = I$ und der Ketteregel
   gilt
   \[J_f(x) · J_{f^{-1}}(f(x)) = I ⇒ J_{f^{-1}}(f(x)) = (J_f(x))^{-1}\]
   #+end_proof
   #+begin_ex latex
   Transformation der Polarkoordinaten auf kartesische Koordinaten. Polarkoodinaten: $(r, θ) \to$ kartesische Koordinaten $(x_1, x_2)$.
   ...
   $f$ ist also auf $D = ℝ_+ × ℝ$ regulär. Nach dem Satz über Umkherabbildung ist $f$ also überall in $D$ lokal umkehrbar
   ...
   $f: U \to V$ wit $U := ℝ_+ × (-\frac{π}{2}, \frac{π}{2}), V := ℝ_+ × ℝ$ ist bijktiv
   \[f^{-1}(x_1, x_2)(\sqrt{x_1^2 + x_2^2}, \arctan(\frac{x_2}{x_1}))\]
   #+end_ex
** Extremalaufgaben mit Nebenbedingungen
   Sei $f: D \to ℝ$ und $g: D \to ℝ$ differenziebare Funktionen auf einer offenen Meng $D ⊂ ℝ^n$. Wir suchen $\hat x ∈ D$, sodass
   ...
   #+begin_thm latex
   Sei $D ⊂ ℝ^n$ offen und $f, g: D \to ℝ$ stetig differenzierbar. Ferner sei $\hat x ∈ D$ ein Punkt, in dem $f$ ein lokales Extremum unter der Nebenbegingung $g(\hat x) = 0$ hat. Das heißt
   ...
   #+end_thm
   #+begin_proof latex
   Wegen $∇ g(\hat x) \neq 0$ können wir (nach evetueller Umnummerierung der Koordinaten) annehmen, dass $\partial_n g(\hat x) \neq 0$
   ...
   #+end_proof
   #+begin_remark latex
   Jedes lokale	Minimum $\v x$ der Funktion $f$ unter der Nebenbedingung $g(\hat x) = 0$ korrespondiert zu einem sogenanntem "stationären Punkt der Lagrange Funktion"
