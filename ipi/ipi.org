	...
	2. Funktionen-Templates: es gab schon function overloadingg
	   ...
	   - mit templates reicht eine Implementation
		 ...
		 - Bespiel 3: checken, ob Container sortiert ist
		   ...
		   // == std::is_sorted
		   #+END_SRC
		 - Bemerkung: Complier-Fehlermeldungen bei Template-Code sind oft schwer zu interpretieren, $\implies$ Erfahrug nötig aber: Compiler werden darin immer besser, besonders clang-comiler
...
** Bestimmung der Effizienz
   ...
   2. unabhängig von Hardware benutz man das Konzept der algorithmischen Komplexität
*** wall clock
   ...
   - Fausregel: Messug zwischen $\SI{0.02}{\second}$ und $\SI{3}{\second}$
...
*** algorithmische Komplexität
	...
	2. vereinfache $f(n)$ in eine einfache Formel $g(n)$, die dasselbe wesentliche Verhalten hat. Die Vereinfachung erfolgt mittels *$O$-Notation* und ihren Verwandten
	   Gegeben: $f(n)$ und $g(n)$
	   1. $g(n)$ ist eine asymptotische (für große $n$) overe Schranke für $f(n)$ ("f(n) \leq g(n)"), $f(n) \in O(g(n))$ "$f(n)$ ist in der Komplexitätsklasse $g(n)$", wenn es ein $n_0$ (Mindestgröße) gibt
	...
	3. $O(f(n)) \cdot O(g(n)) \in O(f(n) g(n))$ \hfill Miltiplikationsregel
	...
	- $O(n \log(n))$ \hfill{"n log n", "loglinear" "linearithmisch"} \\
	  Beispiel: std::sort
	- $O(n^2)$ \hfill "quardatische Komplexität"
	- $O(n^p)$ \hfill "polynomielle Komplexität"
	- $O(2^n)$ \hfill "eexponentielle komplexität"
	#+begin_ex latex
...
*** Anwendung
	1. Fibonacci-Zahlen: $f_k = f_{k - 2} + f_{k - 1}$
	   ...
	   #+end_src
	   - sehr ineffizent, weil alle Fibonacci-Zahlen $< k$  mehrmals berechnet werden
