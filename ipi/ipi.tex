...
\begin{enumerate}
\item bei der Suche die gewünschte Spicherzelle gefunden wird $\implies$ benutze "sprechende Namen"
...
\item wir hatten dynamische Arrays std::string, std::vector, assozaitive Arrays std::map, std::unordered$\backslash$$_{\text{map}}$
\item std::set, std::unordered$\backslash$$_{\text{set}}$: Menge, jedes Element ist höchstens einmal enthalten zum Beispiel Duplikate
...
\item std::deque ("double-ended queue") gleichzeitig stack und queue, push, pop$\backslash$$_{\text{front}}$(), pop$\backslash$$_{\text{back}}$()
\item std::priority$\backslash$$_{\text{queue}}$, push() und pop() - Element mit höchster niedrigster Priorität (user defined)
\end{itemize}
\section{Iteratoren}
\label{sec-12}
\begin{itemize}
...
\item Einfügen des k-ten Elements an richtiger Stelle $\rightarrow$ Erzeuge Lücke an richtiger Position duch verschieben von Elementen nach rechts
...
Komplizierte Fälle (Argument teilweise Template, teilweise hard$_{\text{coded}}$) $\implies$ für Fortgeschrittene
\end{itemize}
