...
\begin{enumerate}
\item weggelassene Klammer weider einfüger
...
\label{sec-5-2}
\begin{itemize}
\item bestehen aus Knoten unmd Kanten (Kreise und Pfeile)
...
\item entferne temporärdessen Elternkante, dadurch wird der Knoten temporär zu einer Wurzel, dieser Knoten mit allen Nachkommen bildet wieder einen Baum (Tailbaum des Orginalbaumes)
\end{itemize}
...
\end{itemize}
\item Compiler oder Interpreter übersetzen Hoch- in Maschienensprache
\end{itemize}
\subsection{Umwandlung in Maschienensprache}
\label{sec-6-1}
\begin{enumerate}
...
\label{sec-7}
\begin{itemize}
\item bei Maschienensprache wreden Zweischeergebnisse in Speicherzellen abgelegt
...
\item Speicherzellen und Namen sind nützlich um Argumente an Fuktionen zu übergeben $\Rightarrow$ Definition eigener Funktionen
\begin{minted}[]{cpp}
function sq(x) {
   return x * x
}
