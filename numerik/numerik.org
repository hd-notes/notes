...
** Interpolation von Funktionen
   ...
   *Bezeichnungen:*
   ...
   Wiedeholung: Fehlerdarstellung Lagrange-Interpolation. Sei $f∈ C^{n + 1}[a, b]$. $∃ ξ_x ∈ \overline{(x_0, \dots, x_n, x)}$
   \begin{align*}
   f(x) - p(x) &= \frac{f^{(n + 1)}(ξ_x)}{(n + 1)!} \prod_{j = 0}^{n}(x - x_j) \\
   f(x) - p(x) &= f[x_0, \dots, x_n, x] \prod_{j = 0}^n (x - x_j) \\
...
** Spline-Interpolation
   Problem: Oszillationen das Interpolationspolynoms, wenn man Stützstellen nicht geeignet wählen kann.
   ...
   4 Koeffizientena uf jedem der $n$ Intervalle ergeben $4n$ Freiheitsgrade. Zur Bestimmung:
   ...
   Seien $s_n^{(1)}$ und $s_n^{(2)}$ inperpolierende Splines $⇒ s = s_n^{(1)} - s_n^{(2)} ∈ N$. Für $ω ∈ N$ beliebig:
   \begin{align*}
