...
*** Ausdehung
	...
	\[V_N = \frac{1}{2}N (8V_a) = 4N V_a = 4(n N_a) V_a \equiv n b\]
	mit:
	- $b = 4 N_A V_a$
	- $V_a = \frac{4}{3}\pi r^3$
    Das Kovolumen muss vom verfügbaren Volumen für Gasteilchen abgezogen werden. Verlust an freiem Volumen erhöht Zahl der Wandanstöße $\implies$ Druckerhöhung
*** Wechselwirkung
	Beschreibung der Wechselwirkung zwischen Gasteilchen dur Leenard-Jones-Potential $\implies$ Langreichweitige Anziehungskraft.
