...
* Numerical Integration
  ...
  #+caption: Plot of $y_n$ for forward and backward integration, some values are intentionally left out to show more details of the small values
  #+ATTR_LATEX: :placement [H]
  #+RESULTS:
  [[file:test.eps]]

  For small $n$ the forward iteration works fine, but at some point it starts to give wrong values, such as negative and very big values, due to roundoff errors.
  However using the backward iteration works fine, and the error of an arbitrary (random) initial condition vanishes after some steps.
* Additional notes
