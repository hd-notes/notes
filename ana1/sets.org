...
***** Bemerkung
	  ...
	  - injectiv, $\Forall x_1,x_2\in X: f(x_1) = f(x_2) \implies x_1 = x_2$
...
*** Definition 1.42
	...
	- monoton $\begin{cases} \text{wachsed} & x \leq y \implies f(x) \leq f(y)  \\ \text{fallend} & x \leq y \implies  f(x) \leq f(y) \end{cases}$
