...
** Umgebung
   ...
   \end{align*}
   Sei $A = \mathbb{Q}$, dann $\bar A = \mathbb{R}$, $\partial A =\mathbb{R}$ denn in jedem \(\eps\)-Intervall um eine rationale Zahl gibt es sowohl rationale als auch irrationale Zahlen
   #+end_ex
   #+begin_remark latex
   \mbox{}
