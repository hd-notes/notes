...
\begin{proof}
\begin{enumerate}
\item Sei $\varepsilon > 0$ vorgegeben
\begin{align*}
...
\label{sec-4-7}
\begin{defn}[2.19]
$A\subseteq \mathbb{K}$ heißt Umgebung von $a\in\mathbb{K} \Leftrightarrow \Exists \varepsilon > 0 I_\varepsilon (a) \subseteq A$
\end{defn}
\begin{conc}[2.20]
...
\begin{proof}
\begin{enumerate}
\item Für irgendein \[i_0 \in I \Exists \varepsilon > 0: I_\varepsilon (a) \subseteq U_{i_0} \subseteq \bigcup_{i \in I} U_i\]
\item Es gilt nach Voraussetzung $\varepsilon_1, \ldots \varepsilon_n > 0$ mit $I_{\varepsilon_i}(a) \subseteq U_i$ für $i = 1,\ldots,n$. Folglich gilt für $\varepsilon:= \min \{\varepsilon_1, \ldots, \varepsilon_n\} > 0,~I_\varepsilon(a) \subseteq U_i (\Forall i = 1,\ldots,n) \Rightarrow I_\varepsilon (a) \subseteq U_1 \cap \ldots U_n$
\item Nach Vorraussetzung gibt es für eine Umgebung $U$ von $a$ ein $\varepsilon > 0$ mit $I_\varepsilon(a) \subseteq U$ \\
      $V:= I_{\frac{\varepsilon}{2}}(a) \subseteq U$ ist ebenfalls Umgebung von $a$ und $\Forall y\in V$ gilt
\[I_{\frac{\varepsilon}{2}} \subseteq I_\varepsilon(x) \subseteq U,~\text{denn}~ \underbrace{\abs{y - z}}_{z\in I_{\frac{\varepsilon}{2}}} < \frac{\varepsilon}{2} \Rightarrow \abs{x - z} \leq \abs{x - y} + \abs{x - z} < \varepsilon\]
\end{enumerate}
\end{proof}
\begin{defn}[2.21]
\mbox{}
...
\item $A\subseteq \mathbb{K}$ ist offen $\Leftrightarrow \Forall a\in A$ ist $A$ die Umgebung von $a$ \\
      (in $\mathbb{R}\Forall a\in A \Exists \varepsilon > 0 I_\varepsilon (a) \subseteq A$)
...
\bar A &= [a,b] \\
\partial A &= \{ a,b \} \\
\Forall \varepsilon > 0 I_\varepsilon(a) \cap (a,b\right] \neq \emptyset \\
I_\varepsilon (a) \cap \mathbb{R} \setminus (a,b\right] \neq \emptyset
\end{align*}
Sei $A = \mathbb{Q}$, dann $\bar A =\mathbb{\mathbb{R}}$, $\partial A =\mathbb{R}$ denn in jedem \$$\varepsilon$-\$Intervall um eine rationale Zahl gilbt es sowohl rationale als auch irrationale Zahlen
\end{ex}
\begin{remark}
\mbox{}
\begin{itemize}
...
\item \[a = \lim_{n\to\infty} \inf a_n \Leftrightarrow \Forall \varepsilon \begin{cases} \string(1\string) \{n \mid \abs{a - a_n} < \varepsilon\} ~\text{ist unendlich (weil $a$ Häufungswert ist)} \\ \string(2\string) \{n \mid a_n < a - \varepsilon\} ~\text{ist endlich ($a$ ist kleinste Häufungswert)}\end{cases}\]
\end{enumerate}
\end{remark}
\begin{ex}[2.24]
\begin{align*}
...
\end{itemize}
\end{remark}
\subsection{Undendliche Summen (Reigen)}
\label{sec-4-8}
\begin{defn}[2.19]
...
\begin{align*}
S_{2^{n + 1}} = \sum_{k = 1}^{2^{n + 1}} \frac{1}{k} = 1 + \frac{1}{2} + \sum_{j = 0}^n \sum_{k = 2^j + 1}^{2^{j + 1}} \frac{1}{k} \geq 1 + \frac{1}{2} + \sum_{j = 0}^{n}\underbrace{\sum_{k = 2^j +1}^{2^{j + 1}}}_{\mathclap{2^j~\text{Summanden}}} \frac{1}{2^{j + 1}} \\
= 1+ \frac{1}{2} + \sum_{j = 1}^n 2^{j} \frac{1}{2^{j + 1}} = 1 + \frac{1}{2} + \sum_{j = 1}^{n} \frac{1}{2} = 1 + \frac{1}{n} + \frac{1}{2}n \xrightarrow{n\to\infty} \infty
\end{align*}
\end{proof}
