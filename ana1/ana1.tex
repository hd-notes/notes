...
\end{itemize}
Mit Hilfe einer Äquivalenzrelation lassen sich die Elemente einer Menge in sogenannte Äquivalenzklassen einordnen: \$[a]:)\{b$\in$ A\mid b\textasciitilde{} a\}
\subsection{Relationen und Abbildungen}
\label{sec-2-3}
\subsubsection{Relationen}
\label{sec-2-3-1}
Unter einer \textbf{Relation} verstehen wir eine Teilmenge $R\subseteq X\tims Y$ wobei $X, Y$ Mengen sind. Für $x\in X$ definieren wir, das \textbf{Bild} von $x$ unter $R$
...
Ist kein Graph einer Funktion $\mathbb{R}\rightarrow\mathbb{R}$, denn \$M$^{\text{ *}}$(x) = \{\sqrt{x},-\sqrt{x}, x$\ge$ 0\}\}
$f$ heißt
\begin{itemize}
...
\item Seien $X,Y$ Mengen, $M\subseteq X\timesY, f:M\rightarrow X$ \\ $f$ heißt Projektion, $f$ surjektiv \[f(M) = \{x\mid \exists y \in Y : (x,y) \in M\} = X\]
\end{itemize}
\end{enumerate}
