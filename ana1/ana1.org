...
*** Satz 1.39
	1. $\abs{\abs{x} - \abs{y}} \leq \abs{x - y}$ \\
	   *Beweis:* \\
	   ...
	   \end{align}
	2. \[\abs{x - y} \leq \eps \Leftrightarrow \begin{cases} x - \eps \leq y \leq x + \eps \\ y - \eps \leq x \leq y + \eps \end{cases}\]
	   *Beweis:* \\
	   \begin{align}
...
*** Lemma 1.51
	...
	Dann gilt
	1. $s = \sup M \Leftrightarrow \Forall \eps > 0 \exists x\in M: s - \eps < x (\geq s)$
	2. $l=\inf M \Leftrightarrow \Forall \eps > 0 \exists x\in M: (l \leq) x < l + \eps$
**** Beweis
	 Wir beweisen 1. \\
