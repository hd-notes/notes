...
** Eigenwerte, Eigenvektoren, Diagonlisierbarkeit
   ...
   Notation: $M^{\dagger} \equiv \bar M^T$ für alle komplexenen Matrizen. Sei $H$ eine hermitesche Matrix ($n × nn$), das heißt $H^{\dagger} = H$.
   ...
   Korollar: Reelle, symmetrische Matrizen $(H = H^{\dagger}, H_{ij} ∈ ℝ)$ können durck orthogonale Transformationen diagonalisiert werden. \\
...
** Freier Kreisel
   ...
   Fall 5: $\sqrt{2 E I_2} < \abs{\v L} < \sqrt{2 E I_1}$ "Gurke", nur Enden sind abgeschnitten $⇒ L$ bewegt sich im körperfisten System auf einer geschlossenen Kurze $⇒$ kräftefreie Präzessions des Kreisels im Laborsystem \\
