...
** Mittelwert
   ...
   #+begin_defn latex
   \[<F> = \frac{1}{N} \sum_{i = 1}^{N} F(i) = \frac{\sum_{i = 1}^{N} F(i)}{\sum_{i = 1}^{N} 1}\]
   #+end_defn
   jetzt für Gas: Meßgröße: $F = F(\v q, \v p)$, außerdem: Verteilungsfunktion $f(\v q, \v p)$
   \[<F> = \frac{1}{N} \sum_{i =¹}^{N} F(\v q_i, \v p_i) = \frac{1}{N} \sum_{α} F(\v q_α, \v p_α) f(\v q_α · \v p_α) (Δq^3 Δ p^3)_α\]
   (Summe über kleine Zellen $α$). Grenzwert kleiner Zellen:
   \[<F> = \frac{1}{N} ∫ \d^3 \v p \d^3 \v q f(\v q, \v p) F(\v q, \v p) = \frac{∫ \d^3 \v p \d^3 \v q f(\v q, \v p) F(\v q, \v p)}{∫\d^3 \v p \d^3 \v q f(\v q, \v p)}\]
   $F, f$ unabhängig von $\v q$:
   \[<F> = \frac{∫ \d^3 \v p f(\v p) F(\v p)}{∫ \d^3 \v p f(\v p)}\]
\subsection*{Maxwell-Bolzmann-Verteilung - Fortsetzung}
   Wähle Observablen $\{F_1, F_2, F_3\} \equiv \v P = \v p$. Behauptung: $<\v F> = \v p_0$. (Nachprüfen für erste Komponente reicht)
   \[<p_1> = \frac{∫ \d^3 \v p p_1 C e^{-A(\v p - \v p_0)^2}}{∫ \d^3 \v p C e^{- A(\v p - \v p_0)^2}}\]
   ...
   \[F(\v p) = \frac{\v p^2}{2m}, ε <\frac{\v p^2}{2m}> = \frac{3}{4 A m}\]
...
** Diffusion
   ...
   \[v \equiv <\abs{\v v}> \sim \frac{Δ y}{Δ t}\]
   $⇒$ Stromdichte:
   \begin{align*}
   ...
   ⇒ \pp{n(\v q)}{t} &\sim \v ∇_q \underbrace{∫ \d^3 \v p f(\v q, \v p) \frac{\v p}{m}}_{<\v v> n = \v j} \sim \v ∇_q \v j
   \end{align*}
...
** Bolzmannverteilung als wahrschenlichste Verteilung
