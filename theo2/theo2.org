...
** Poisson-Klammern und Vektorfelder
   ...
   Zurücz zum speziellen Fall der Hamilton Dynamik:
   ...
   \begin{center}
   Observable $\longleftrightarrow$ Differentialoperator \\
   $\{·, ·\}$	$\longleftrightarrow$ $[D_F, D_G] = \underbrace{D_F D_G - D_G D_F}_{Kommutator}$
   \end{center}
...
** Die Drehimpuls Lie-Algebra in die Hamilton-Mechanik
   ...
   Kommutator tacht $SO(3)$ zur Lie-Algebra:
...
** Satz von Liouville
   ...
   #+end_thm
   Genauere Begrüngung: Wähle zwei Volumina $V, V'$:
   \begin{align*}
...
** Tangential- und Cotangentialraum
   ...
   Zu $V$ gibt es Dualraum $V^{\ast}$. (Raum der linearen Funktionale. Hie heißt dieser Cotangentiaraum $(T_q M)^{\ast}$ auf $V$).
